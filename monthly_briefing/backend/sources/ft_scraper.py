...
USER_DATA_DIR = LOCAL_APP_DATA / "IFC_Monthly_Briefing" / "ft_session"
...
class FTScraper:
    ...
    def scrape(self, date_start: datetime, date_end: datetime) -> List[Dict]:
        """
                    ...
                    if page.get_by_text("Sign In").count() > 0:
                        if page.get_by_text("My Account").count() == 0:
                            ...
                            try:
                                ...
                                # Fallback: Maybe we are logged in but Selector failed?
                        ...
                        for i in range(count):
                            ...
                            if time_element.count() > 0:
                                ...
                                try:
                                    ...
                                    # Date Range Check
                                    # Ensure offset-aware comparison if needed. 
                                    # date_start/end are likely naive or from local.
                                    # Best to compare .date()
                                    if not (date_start.date() <= article_date.date() < date_end.date()):
                                        continue 
                                except Exception:
                                    continue
                            else:
