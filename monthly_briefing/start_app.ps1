...
try {
    ...
    $procIdsOnPort = Get-NetTCPConnection -LocalPort 8001 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique
    foreach ($procId in $procIdsOnPort) {
        try {
            Stop-Process -Id $procId -Force -ErrorAction SilentlyContinue
            Write-Host "Killed process $procId on port 8001" -ForegroundColor DarkGray
        }
        catch {}
    }
    ...
    Write-Host "Monitoring server at http://127.0.0.1:8001/health" -ForegroundColor Cyan
    ...
    while ($true) {
        ...
        $isListening = Get-NetTCPConnection -LocalPort 8001 -State Listen -ErrorAction SilentlyContinue
        
        if (!$isListening) {
            Write-Host "Server port 8001 is not active. Starting server..." -ForegroundColor Yellow
            Start-Process -FilePath $PYTHON -ArgumentList "-m uvicorn main:app --reload --host 127.0.0.1 --port 8001" -WindowStyle Hidden
            Start-Sleep -Seconds 12 # Give it time to bind and warm up AI
        }

        # Health Check
        try {
            $resp = Invoke-WebRequest -Uri "http://127.0.0.1:8001/health" -TimeoutSec 5 -UseBasicParsing -ErrorAction Stop
            ...
            if ($failureCount -ge $maxFailures) {
                ...
                # Hard Cleanup
                $procIdsToKill = @()
                $procIdsToKill += Get-NetTCPConnection -LocalPort 8001 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique
