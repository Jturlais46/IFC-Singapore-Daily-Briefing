...
async def test_filtering():
    ...
    if len(bt_items) > 0 and len(other_items) == 0:
        ...
        print("[FAILURE] Filtering failed or no items returned.")
    ...
    if len(other_items) == 0 and len(items) > 0:
        print("[SUCCESS] Only selected sources fetched.")
    else:
        print("[FAILURE] Found unrequested sources or no items.")
