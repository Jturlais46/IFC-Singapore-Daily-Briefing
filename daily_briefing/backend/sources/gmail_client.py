...
class GmailClient:
    ...
    def list_messages(self, sender: str, after_date: datetime) -> List[Dict]:
        """List messages from a sender after a certain date."""
        query_date = after_date - timedelta(days=1)
        date_str_after = query_date.strftime("%Y/%m/%d")
        
        # Add explicit upper bound to limit to single day (or 2 days window to be safe for timezones, but user asked for strict)
        # after:2023/10/26 means >= 2023/10/27. 
        # before:2023/10/29 means < 2023/10/29.
        # To get strictly target date (e.g. 27th), we want >= 27th and < 28th.
        # So after:26th before:28th.
        
        date_before = after_date + timedelta(days=1)
    ...
    def fetch_news(self, target_date: datetime) -> List[Dict]:
        """Main entry point to fetch and parse emails for a specific date."""
        all_news = []
        
        # Fetch Google Alerts
        print(f"[Gmail] Fetching Google Alerts...")
        msgs = self.list_messages(ALERTS_SENDER, target_date)
        for msg in msgs:
        ...
        msgs = self.list_messages(EXECSUM_SENDER, target_date)
        for msg in msgs:
            content = self.get_message_content(msg['id'])
            if content and content['body']:
